...
export class MockSwapService {
  ...
  FEE_PERCENTAGE = 5; // 5% swap fee
  FEE_RECIPIENT = "0x27cEe32550DcC30De5a23551bAF7de2f3b0b98A0";
  ...
  async getSwapPrice(params) {
    try {
      ...
      return {
        success: false,
        error: `Failed to get swap price: ${error.message}`,
  ...
  formatAmount(amount, decimals = 18) {
    try {
      ...
      console.warn(`Failed to format amount ${amount}: ${error.message}`);
      return "0.0";
    }
  }
   ...
   */
  calculateExchangeRate(fromAmount, toAmount) {
